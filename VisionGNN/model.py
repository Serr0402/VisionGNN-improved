...
class ViGBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x + shortcut
        x = self.droppath2(self.out_layer2(F.gelu(self.in_layer2(x.view(B * N, -1)))).view(B, N, -1)) + x
        return x

class VGNN(nn.Module):
    def __init__(self, out_feature=320, num_ViGBlocks=16, num_edges=9, head_num=1, patch_size=16, image_size=224):
        super().__init__()
        # Calcular dinámicamente in_features y num_patches
        in_features = 3 * patch_size * patch_size
        num_patches = (image_size // patch_size) ** 2

        print(f"Initializing VGNN with in_features={in_features}, out_feature={out_feature}, "
              f"num_patches={num_patches}, num_ViGBlocks={num_ViGBlocks}, num_edges={num_edges}, head_num={head_num}")
...
#Aqui añadi el print para asegurarme aunque en el original no estaba. gracias a esto detecte el bug del patch size
class Classifier(nn.Module):
    def __init__(self, out_feature=320, num_ViGBlocks=16, hidden_layer=1024, num_edges=9, head_num=1, n_classes=10, patch_size=16, image_size=224):
        super().__init__()
        print(f"Initializing Classifier with out_feature={out_feature}, num_ViGBlocks={num_ViGBlocks}, "
              f"num_edges={num_edges}, head_num={head_num}, n_classes={n_classes}, patch_size={patch_size}")

        self.backbone = VGNN(out_feature, num_ViGBlocks, num_edges, head_num, patch_size, image_size)
        num_patches = (image_size // patch_size) ** 2
